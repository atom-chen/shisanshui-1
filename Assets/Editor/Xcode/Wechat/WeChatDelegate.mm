...
extern "C"
{
    ...
    void WechatShareGameRoom(const char* url,const char* title,const char* desc,const char* roomId,const char* roomToken, const char* icon_path,int type,IntCallBack cb);
...
- (void)onOpenURL:(NSNotification*)notification
{
    ...
    if(url != nil)
    {
        ...
        if([urlStr rangeOfString:@"xianliao"].location != NSNotFound){
            [SugramApiManager handleOpenURL:url];
        }
        else{
...
const char* GetShemesArgs()
{
    ...
    if(path != nil && path.length != 0)
    {
        ...
        if([url query] == NULL){
            ...
            //GetShemesArgs {"/path":"joinRoom","query":"id=705775"}
            //GetShemesArgs {"path":"/joinRoom","query":"id=974868"}
            json = @"{\"path\":\"/%@\",\"query\":\"id=%@\"}";
            _mSchemesArgs = [NSString stringWithFormat:json, roomToken, roomId];
