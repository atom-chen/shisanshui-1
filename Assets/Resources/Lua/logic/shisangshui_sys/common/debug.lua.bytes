...
function echoError(fmt, ...)
    if fmt == nil then
        echo( debug.printback("", 2) )
        return
    end
    echo(string.format("[ERR] %s%s", string.format(tostring(fmt), ...), debug.printback("", 2)))
    --os.exit(0)
end
...
function dump(object, label, isReturnContents, nesting)
    ...
    local printback = string.split(debug.printback("", 2), "\n")
    echo("dump from: " .. string.trim(printback[3]))
