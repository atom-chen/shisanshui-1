...
local On_NETWORK_CONNECTION_CLOSE = nil
local On_LOGOUT_AND_CONNECT_LOGIN = nil
...
function RegisitEvents()
	Notifier.regist(cmdName.NETWORK_CONNECTION_CLOSE, On_NETWORK_CONNECTION_CLOSE)
end
...
function UnRegistEvents()
	Notifier.remove(cmdName.NETWORK_CONNECTION_CLOSE, On_NETWORK_CONNECTION_CLOSE)
end

--[[--
 * @Description: 处理网络链接断开
 ]]
function On_NETWORK_CONNECTION_CLOSE()
	if (this.state_current ~= nil) then
		this.state_current:HandleOnNetworkClose()
	end
end

function On_LOGOUT_AND_CONNECT_LOGIN()
	if (this.state_current ~= nil) then
		this.state_current:HandleReConnectAndLogin()
	end
end
