...
function DebugLog(func, str, filterCode)
	if (DEBUG_print == nil or DEBUG_print == false) then
		return
	end
	func(str, filterCode)
end
...
function addPressedCallbackSelf(parentTrans, controlName, callback, self)
	...
	if (trans ~= nil) then
		...
		if self ~= nil then 
			...
			UIEventListener.Get(btnObj).onPress = function(go, isPress) callback(self, go, isPress) end
		else
...
--[[--
 * @Description  是否在副本内（包括单人，多人）
 ]]
function IsInLevel()
	local isInSingleLevel = level_sys_single.IsInLevel()
	local isInMultiLevel = level_sys_multi.IsInLevel()

	if (isInSingleLevel or isInMultiLevel) then
		return true
	else
		return false
	end
