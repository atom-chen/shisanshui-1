...
-- 场景和UI的绑定关系
local ui_bindings = {}
ui_bindings[scene_type.LOGIN] = {["ui_name"] = {"Login/login_ui"}, ["uiObj"] = {}}
ui_bindings[scene_type.HALL] = {["ui_name"] = {"Hall/hall_ui"}, ["uiObj"] = {}}
ui_bindings[scene_type.HENANMAHJONG] = {["ui_name"] = {"Mahjong/mahjong_ui"}, ["uiObj"] = {}}
ui_bindings[scene_type.FUJIANSHISANGSHUI] = {["ui_name"] = {"shisangshui/shisangshui_ui"},["uiObj"] = {}}
--[[--
 * @Description: 初始化  
 ]]
function this.init()
...
function this.DoLoadScene(sceneId, sceneType, newSys)
	...
	if sceneType==scene_type.LOGIN then
		 SceneMgr.Instance:LoadScene(currSceneID, sceneId, false, 0, loadingTips)
	else
		--如果有需要，需计算出预加载的物体id(第三个参数)
...
function this.GetLoadingTips(sceneId, sceneType)
	...
	if sceneType == scene_type.LOGIN then
		...
		-- 	loadingTips = "返回登录"
		-- elseif currSceneType == scene_type.NONE then --
		-- 	loadingTips = "正在为您加载，即将进入登录"
		-- end
		...
		if ui_bindings[mainUIType] ~= nil then
			...
			if mainUIType == scene_type.HALL and (not IsNil(hallObj)) then
				...
				for k, v in pairs(ui_bindings[mainUIType]["ui_name"]) do
					local abi = {}
					abi.mainObject = newNormalObjSync("Prefabs/UI/"..v, typeof(GameObject))
